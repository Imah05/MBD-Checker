...
This repository contains the code used for the computer-aided computations in the thesis "The Maker-Breaker Domination Game and the Minimum degree Question".
...
### `game_state.cpp`/`game_state.h`
  - Class for game states (see thesis)
  - `outcome` (see Algorithm 1 in the thesis)
...
### `pccgs.cpp`/`pccgs.h`
  - Class for partially completed core game states (see thesis)
  - `completionOutcome` (see Algorithm 4 in the thesis)
...
### `sequence_generator.cpp` (contains a `main`)
  - `generateSeqs` (see Algorithm 4 in the thesis) 
...
### Several `check_*.cpp` (all containing a `main`)
  ...
  * `main` in `check_cores.cpp`: Reads cores (of reduced graphs) in graph6 format from stdin. Reports to stderr each core, which has a completion on which Staller wins going second and which has a degree sequence occurring in `deg_seqs.txt`. 
